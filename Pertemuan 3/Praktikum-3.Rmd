...
```{r}
library(readxl)
dt <- read_xlsx("D:\\Mine\\Kuliah Journey\\Semester 4\\Analisis Regresi\\anime_2022_clean.xlsx", sheet="Sheet1")
dt
```
...
Berdasarkan perhitungan, didapat dugaan persamaan regresi linear sebagai berikut. Dugaan persamaan tersebut dapat pula disebut sebagai nilai harapan dari peubah respons Y (Score)

$$E[\hat{Y}] = \hat{Y} = 7.572 - 0.0001093 X$$
...
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{0.462723} \approx 0.6802375$
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar 7.572 dan -0.0001093. Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [7.026484 ,  8.118098] dan untuk parameter $β_1$ adalah [-0.000123824 ,  -9.469669e-05]
...
Misalkan kita ingin menduga nilai rataan (harapan) amatan ketika nilai x=100. Kita dapat memanfaatkan fungsi `predict` untuk mendapatkan nilai dugaan dan selang kepercayaannya sebagai berikut.

```{r}
amatan.diduga <- data.frame(x=100)
predict(model, amatan.diduga, interval = "confidence")
```

Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=100 adalah 7.561365. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=100 berada dalam selang 7.459439 hingga 7.663292.
...
Misalkan kita ingin menduga nilai individu amatan ketika nilai x=100. Kita dapat memanfaatkan fungsi `predict` untuk mendapatkan nilai dugaan dan selang kepercayaannya sebagai berikut.

```{r}
predict(model, amatan.diduga, interval = "prediction")
```

Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=100 adalah 7.561365. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=100 berada dalam selang 6.221156 hingga 8.901574.